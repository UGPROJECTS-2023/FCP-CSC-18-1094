...
?>
	...
	<div class="container">
               <div class="span12">
						...
						<th>ACTION</th>
                                ...
                                <tbody>
                                    ...
                                    $query = mysqli_query($con, "select * from result_sheet WHERE department = '$eo_department' AND adm_type = 'pe'") or die(mysql_error());
			...
			$cutsql = mysqli_query($con, "SELECT pe_cut_off,pe_total_no FROM department WHERE dept ='$eo_department'") or die(mysql_error());
		while ($cutrow = mysqli_fetch_array($cutsql)) {
			$cut = $cutrow['pe_cut_off'];
			$total_no = $cutrow['pe_total_no'];
			$merit = 0.4 * $total_no; 
			$catchment = 0.3 * $total_no; 
			$elds = 0.3 * $total_no; 
			} 
																		
									$merit_adm =	mysqli_query($con, "UPDATE result_sheet SET status = '1' WHERE (department = '$eo_department' AND adm_type = 'pe' AND pscore >= $cut)  ORDER BY pscore DESC LIMIT $merit ") or die(mysql_error());
									$catchment_adm =	mysqli_query($con, "UPDATE result_sheet SET status = '1' WHERE (department = '$eo_department' AND adm_type = 'pe' AND pscore >= $cut AND status = '0' AND catchment = '1')  ORDER BY pscore DESC LIMIT $catchment ") or die(mysql_error());
									$elds_adm =	mysqli_query($con, "UPDATE result_sheet SET status = '1' WHERE (department = '$eo_department' AND pscore >= $cut AND adm_type = 'pe' AND status = '0' AND elds = '1')  ORDER BY pscore DESC LIMIT $catchment ") or die(mysql_error());
									if (($merit_adm)&&($catchment_adm)&&($elds_adm)){ ?>
										<script>
										window.location = "dept_admission.php";
										</script>
								...
								<?php	}}
																		...
																		$eo_department = $_SESSION["department"];
mysqli_query($con, "DELETE FROM result_sheet WHERE department = '$eo_department' AND adm_type = 'pe' ") or die(mysql_error());
